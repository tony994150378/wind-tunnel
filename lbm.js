  ...
  class WindTunnel {
    ...
    static shapeMask(w, h, kind, opts = {}) {
      ...
      if (kind === 'cylinder') {
        ...
        // 主体轮廓（按真实 F1 侧视修正）：
        // 车头 = 前倾楔形鼻锥(左，非半圆/非垂直墙)；车顶 = 座舱平顶(中)；
        // 车尾 = 可乐瓶收腰(右，圆滑下收接尾翼中央支柱)
        fillPoly([
          [x0 - 0.02 * L, floor],                 // 车头底盘最前端（略前伸，接前翼）
          [x0, yc + Hh * 0.62 + off],             // 鼻尖：楔形尖，向前下方接前翼，非半圆/非垂直墙
          [x0 + 0.16 * L, yc + Hh * 0.18 + off],  // 鼻锥向后上方倾斜升到鼻根
          [x0 + 0.30 * L, yc - Hh * 0.34 + off],  // 座舱前升
          [x0 + 0.40 * L, yc - Hh * 0.52 + off],  // 座舱圆顶（车顶最高，做平）
          [x0 + 0.50 * L, yc - Hh * 0.50 + off],  // 座舱后/airbox：车顶左段平
          [x0 + 0.64 * L, yc - Hh * 0.22 + off],  // 引擎盖向车尾下收（可乐瓶颈部起点）
          [x0 + 0.80 * L, yc + Hh * 0.06 + off],  // 车尾上轮廓继续下收
          [x1, yc + Hh * 0.24 + off],              // 车尾顶（接尾翼中央支柱）
          [x1, floor],                             // 车尾底盘
        ]);
        // 前翼（车头，向左下突出，贴近地面）
        fillPoly([
    ...
    step() {
      ...
      this._applyBC();
      // 出口采用零梯度开放边界（_applyBC 中 x=w-1 复制 x=w-2），
      // 不额外施加 sponge 吸收区，避免在画面中形成可察觉的竖向过渡带。
      // 渲染时会对最右段做 alpha 淡出，让尾流蓝色自然过渡到背景。
      this._computeCurl();
      this.stepCount++;
